...
if ($row['email'] == $email and $row['pwd'] == $psw) {
    ...
    if ($row['traducteur'] == 1) {$sql2 = "SELECT * FROM `traducteur` WHERE `user`='$USR'";
        ...
        $home_model->Session($row['traducteur'], $row2['user'], $row2['email'], $row2['nom'], $row2['prenom'], $row['pwd'], $row2['telephone'], $row2['adresse'], $row2['commune'], $row2['wilaya'], $row2['fax'], $row2['ref1'], $row2['ref2'], $row2['ref3'], $row2['asser_justif'], $row2['cv'], $row2['type']);
        header("Location: ../index.php");
    }
;
